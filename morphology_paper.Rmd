...
The morphological characteristics of both species are presented in figure \ref{fig:schematic}. *Laminaria pallida* is characterised by a single smooth blade which is divided longitudinally into sections, and develops from a single meristematic region located at the junction between the blade and the stipe [@Dyer2018-rv]. This species has a solid stipe but develops a hollow stipe along the west coast northward and into Namibia [@Rothman2017-pn]. *Ecklonia maxima* consists of a single primary blade which develops above a gas-filled float and a hollow stipe below. Secondary blades are produced laterally from the primary blade from several meristematic regions along the margins of the primary blade, known as digits [@Dyer2018-rv]. Both species are held to the substrate by finger-like haptera, collectively known as the holdfast. 
...
```{r deep ecklonia wave, echo=FALSE, fig.cap='\\label{fig:deep ecklonia wave}Boxplot of morphological characteristics for deep population *E. maxima* grouped by wave exposure.', fig.height=8, fig.width=8, warning=FALSE}
...
```{r shallow ecklonia temp, echo=FALSE, fig.cap='\\label{fig:shallow ecklonia temp}Boxplot of morphological characteristics for shallow population *E. maxima* grouped by temperature regime.', fig.height=7, fig.width=8, message=FALSE, warning=FALSE}
ggarrange(EST1,EST2,EST3,EST4,EST5,EST6,EST7,EST8,EST9,EST10, labels = "AUTO", common.legend = FALSE, legend = "bottom", ncol = 3, nrow = 4)
```

```{r shallow ecklonia wave, echo=FALSE, fig.cap='\\label{fig:shallow ecklonia wave}Boxplot of morphological characteristics for shallow population *E. maxima* grouped by wave exposure.', fig.height=7, fig.width=8, warning=FALSE}
...
```{r rda plots, echo=FALSE, fig.height=8, fig.width=8, message=FALSE, warning=FALSE, fig.cap="\\label{fig:rda plots}RDA output for *L. pallida* (A), deep *E. maxima* (B) and shallow *E. maxima* (C)."}
ggarrange(lam_rda, rda_eck_deep, rda_eck_shallow, common.legend = TRUE, legend = "bottom", labels = "AUTO", ncol = 2, nrow = 2)
```
