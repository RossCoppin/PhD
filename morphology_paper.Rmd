...
output:
  ...
  html_document:
    df_print: paged
  word_document:
    reference_docx: morphology_paper_files/frontiers_template.docx
papersize: A4
tables: yes
fontsize: 10pt
---
...
lam_data <- read.csv("data/morph_data/lam_morph_data.csv", sep = ";") %>%
  ...
  mutate(site = replace(site, site == "Bordjies North", "Bordjies Deep")) %>%
...
positions_temp <- c("Betty's Bay", "Roman Rock","Kalk Bay", "A-Frame", "Miller's Point", "Baboon Rock",  "Batsata Rock", "Buffelsbaai","Buffelsbaai South","Olifantsbos","Scarborough", "Soetwater", "Kommetjie","Hout Bay","Oudekraal","Sea Point","St Helena Bay")
...
```{r wave power map, echo=FALSE, fig.cap='Schematic of *E. maxima* on the left and *L. pallida* on the right. (Dyer 2018)'}
library(png)
library(grid)

img_wavepower <- readPNG("figures/CapePen_Total.PNG")
grid.raster(img_wavepower)
```
...
Significant differences were found between cool temperate and warm temperate regimes for all *L. pallida* morphological differences with lamina length, number of digits and thallus mass the exceptions. Certian morphological characteristics such as stipe mass, stipe length, total length and stipe diameter had significanlty higher means for the cool temperate regimes when compared to the warm temperate regime. Conversely, lamina weight and lamina thickness has significantly lower means for the cool temperate regime compared to the warm temperate regime. When *L. pallida* morphological characteristics were grouped to wave exposure categories kruskal-wallis tests revealed significant differences amoungst categories for all morphological characteristics with lamina length, number of digits and thallus mass the exceptions. The morphological characteristics that were significantly different amoungst sites exibited similar patterns, with increasing mean values from the full sheltered to extremely sheltered categories and then a decrease in mean values for the remainder of the categories. Two morphological characteristics, namely lamina thickness and stipe diameter, both exibit higher variations for the exposed categories. 
...
```{r laminaria temp plots, echo=FALSE, warning=FALSE, fig.cap='Boxplots of *L. pallida* morphological characteristics group by temperature regime.', message=FALSE, warning=FALSE, include=FALSE, fig.height=8, fig.width=7}
ggarrange(LT1, LT2,LT3,LT4,LT5,LT6,LT7,LT8,LT9, labels = c("AUTO"), common.legend = TRUE, legend = "bottom")
```

```{r lamanaria wave plots, echo=FALSE,warning=FALSE, fig.cap='Boxplots of *L. pallida* morphological characteristics group by wave exposure category.', fig.height=8, fig.width=8}
ggarrange(LW1, LW2,LW3,LW4,LW5,LW6,LW7,LW8,LW9, labels = c("AUTO"), common.legend = TRUE, legend = "bottom")
```

All deep *E. maxima* morphological characteristics showed differences between cool temperate and warm temperate regimes except for frond mass, frond length and stipe mass. Mean values for primary length, primary width and stipe circumference were all significantly higher for the cool temperate regime than the warm temperate regime. The remaining morphological characteristics which showed significant differences between regimes exhibited significanlty lower values for the cool temperate regime compared to the warm temperate regime. Significant differences amoungst wave exposure categories were found for all morphological characteristics of deep *E. maxima* populations. Certian morphological characteristics showed a gradual increase in mean value with inncreasing wave exposure. The remaining morphological characteristics exhibited decreased values for only the higher exposure categories and then a sharp increase in value for the highest wave exposure category. In all cases, mean values for the highest wave exposure level (very exposed) was greatest compared to other levels of exposure. 
...
```{r deep ecklonia temp plot, message=FALSE, warning=FALSE, fig.cap ='Boxplot of morphological characteristics for deep population *E. maxima* grouped by temperature.',fig.height=8, fig.width=8}
ggarrange(ET1,ET2,ET3,ET4,ET5,ET6,ET7,ET8,ET9,ET10, labels = "AUTO", common.legend = TRUE, legend = "bottom")
```

```{r deep ecklonia wave plots, warning=FALSE,fig.cap ='Boxplot of morphological characteristics for deep population *E. maxima* grouped by wave exposure.',fig.height=8, fig.width=8}
...
```{r shallow ecklonia temp plots, echo=FALSE, message=FALSE, warning=FALSE, fig.cap ='Boxplot of morphological characteristics for shallow population *E. maxima* grouped by temperature regime.'}
ggarrange(EST1,EST2,EST3,EST4,EST5,EST6,EST7,EST8,EST9,EST10, labels = "AUTO", common.legend = FALSE, legend = "bottom")
```

```{r echo=FALSE, warning=FALSE, warning=FALSE, fig.cap ='Boxplot of morphological characteristics for shallow population *E. maxima* grouped by wave exposure.'}
ggarrange(ESW1,ESW2,ESW3,ESW4,ESW5,ESW6,ESW7,ESW8, ESW9, labels = "AUTO", common.legend = FALSE, legend = "bottom")
```

```{r rda analysis lamanaria, eval=FALSE, include=FALSE}
...
lam_mean <- lam_df %>% 
  ...
  summarise_all(funs(mean), na.rm = T) %>% 
  filter(site != "Roman Rock") %>% 
  filter(site != "Miller's Point") %>%  #the scipt keeps dropping it when combing the site list with the metadata
  ungroup()
...
#remove date column and extra Betty's Bay
abio_lam <- abio_lam[-c(5) , -c(2)]
...
lam_meta_rda <- merge(lam_sites, kelp_sites, by = "site")
lam_meta_rda <- lam_meta_rda[-c(1,9,11,14), ]
  
# Plot RDA
lam_rda <- ggord(rda_lam, lam_meta_rda$region, ellipse = TRUE, hull = FALSE, grp_title = "Region", ptslab = TRUE, parse = TRUE, addsize = 3.5, ext = 0.9) 
```

```{r rda deep ecklonia processing, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
...
eck_meta_rda <- merge(eck_sites, kelp_sites, by = "site")
...
```{r rda shallow ecklonia processing, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}

#filter to select only deep population ecklonia and remove Millers B and C for now...
eck_df_sh <- eck_data %>%
  filter(population == "Shallow") %>% 
  mutate(site = replace(site, site == "Buffelsbaai", "Buffelsbaai South")) %>% 
  mutate(site = replace(site, site == "Miller's Point C", "Miller's Point")) %>% 
  mutate(site = replace(site, site == "Miller's Point B", "Baboon Rock")) %>% 
  filter(site != "St James") %>% 
  filter(site != "Slangkop") %>% 
  filter(site != "Miller's Point A")
...
#use site eck_site to align appropriate data from abio_clim data from the lamanaria processing section
abio_clim_temp <- abio_clim %>% 
  mutate(site = replace(site, site == "Sea Point", "Bakoven"))
  
abio_eck_sh <- merge(abio_clim, eck_sites_sh, by = "site") 
...
# Parsimonious RDA
rda_eck_shallow <- rda(bio_eck_sh ~ hs_max + hs_mean + sd_temp + mean_temp, data = abio_eck_sh)
...
# Plot RDA
rda_eck_shallow <- ggord(rda_eck_shallow,ellipse = TRUE, hull = FALSE, grp_title = "Sites", ptslab = TRUE, parse = TRUE, addsize = 3.5, ext = 0.9, coord_fix = TRUE)  # looking at the raw code, this is plotting the 'wa scores', the blue 

```

```{r rda plots, eval=FALSE, include=FALSE}
ggarrange(lam_rda, rda_eck_deep,rda_eck_shallow, common.legend = TRUE, labels = "AUTO", legend = "bottom")
```
