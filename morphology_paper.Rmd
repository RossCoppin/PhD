...
lam_data <- read.csv("data/morph_data/lam_morph_data.csv", sep = ";") %>%
  ...
  mutate(site = replace(site, site == "Bordjies North", "Bordjies")) %>%
...
```{r rda deep ecklonia processing, echo=FALSE, message=FALSE, warning=FALSE}
...
# Plot RDA
rda_eck_deep <- ggord(rda_eck, ellipse = TRUE, hull = FALSE, ptslab = TRUE, parse = TRUE, addsize = 3.5, ext = 0.95)  # looking at the raw code, this is plotting the 'wa scores', the blue 
```
...
# Plot RDA
rda_eck_shallow <- ggord(rda_eck_shallow, eck_meta_sh$region,ellipse = TRUE, hull = FALSE, grp_title = "Sites", ptslab = TRUE, parse = TRUE, addsize = 3.5, ext = 0.9, coord_fix = TRUE)  # looking at the raw code, this is plotting the 'wa scores', the blue 

```

```{r rda plots, echo=FALSE}
