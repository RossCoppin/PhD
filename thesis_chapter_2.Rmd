...
geom_polygon(data = south_africa_coast_hi_res, aes(x = lon, y = lat, group = PID),
  ...
  geom_point(data = study_sites, aes(x = lon, y = lat), color = "red") +
  geom_text(data = study_sites_edit, aes(x = lon, y = lat, label = site), nudge_x = -0.1, nudge_y = -0.01) +
  annotate("text", label = "WARM\nTEMPERATE", x = 19.0, y = -34.5, size = 4.0, angle = 0, colour = "black") +
  annotate("text", label = "COOL\nTEMPERATE", x =17.8, y = -33.5, size = 4.0, angle = 0, colour = "black") +
  annotate("text", label = "Cape\nPeninsula", x = 18.4, y = -34.5, size = 3.0, angle = 0, colour = "black") +
...
map_B <- ggplot() +
  ...
  geom_text(data = study_sites, aes(x = lon, y = lat, label = site), nudge_x = 0.02, nudge_y = 0) +
  coord_cartesian(xlim = c(18.275, 18.525), ylim = c(-34.375, -34.100)) +
  theme(axis.ticks = element_blank(), # Remove tick marks
...
map_combined <- ggplot() +
  ...
  annotation_custom(grob_B,
                    xmin = 4, xmax = 10, ymin = 4.5, ymax = 10)
ggsave(filename = "map_combined.jpg", plot = last_plot(), device = "jpeg", path = "map/", width = 8, height = 8)
```
