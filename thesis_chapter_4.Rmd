...
houtbay <- tidync("OceanParcels/trajectory_data/chapter_4/houtbay.nc") %>%
  hyper_tibble() %>%
...
kommetjie <- tidync("OceanParcels/trajectory_data/chapter_4/kommetjie.nc") %>%
  hyper_tibble() %>%
  dplyr::select(trajectory, lon, lat, time, distance, age) %>%
  dplyr::rename(t = time) %>%
  mutate(t = as.POSIXct(t, origin = "2020-01-01 00:00:00"))
...
soetwater <- tidync("OceanParcels/trajectory_data/chapter_4/soetwater.nc") %>%
  hyper_tibble() %>%
  dplyr::select(trajectory, lon, lat, time, distance, age) %>%
  dplyr::rename(t = time) %>%
  mutate(t = as.POSIXct(t, origin = "2020-01-01 00:00:00"))
...
millers <- tidync("OceanParcels/trajectory_data/chapter_4/millers.nc") %>%
  hyper_tibble() %>%
  dplyr::select(trajectory, lon, lat, time, distance, age) %>%
  dplyr::rename(t = time) %>%
  mutate(t = as.POSIXct(t, origin = "2020-01-01 00:00:00"))
...
heatmap_houtbay <- ggplot() +
  ...
  scale_fill_continuous(type = "gradient", limits = c(1, 130000)) +
...
heatmap_komm <- ggplot() +
  geom_polygon(data = south_africa_coast_hi_res, aes(x = lon, y = lat, group = PID),
               size = 1.0, colour = "black", fill = NA) +
  #geom_bin2d(data = p_RK4, bins = 80, aes(x = lon, y = lat)) +
  geom_hex(data = kommetjie, bins = 80, aes(x = lon, y = lat), legend = FALSE) +  
  scale_fill_continuous(type = "gradient", limits = c(1, 130000)) +
...
ggarrange(heatmap_houtbay, heatmap_komm, heatmap_soetwater, heatmap_millers, labels = "AUTO", common.legend = TRUE)
