...
```{r trajectory plots, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Mean trajectory paths of each exposure scenario over the entire simulation run. Trajectory along the latitudinal and longitudal paths are plotted over time seperatley. Plot A is the longitudinal and plot B the latitudinal trajectories respectively."}
...
lon_plot <- ggplot() +
  geom_line(data = total_mean_traj, aes(x = t, y = lon, color = type), shape = "type") +
  xlab("Time") +
  ylab("Longitude") +
  theme_bw() +
...
lat_plot <- ggplot() +
  geom_line(data = total_mean_traj, aes(x = t, y = lat, color = type)) +
  xlab("Time") +
  ylab("Latitude") +
  theme_bw() +
  guides(fill=guide_legend(title="Type"))
