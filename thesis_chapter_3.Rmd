...
where $X$ is the three-dimensional position of a particle, $v(x,t)$ is the three-dimensional velocity field at the location in the ocean model, $X_b(t)$ is a change in position due to 'behaviour' of the particle. This can range from swimming in fish to sinking or beaching. IN order to investigate the effects of drag components of trajectory of floating macroalgae, this study incorporated drag as the custom behaviour. This study simulated two scenarious; one where the plant is fully submerged and only exposed to seawater drag and the other where parts of the plants are partially exposed resulting in a combination of seawater and air form drag. Unlike previous studies, drag based on shape based coefficients was used in the calculations. 

The energy momemtum equation was used to calculate the drag force exterted on the virtual kelp particle, $$m({d_u/d_t})=F_d + F_w +....$$


The velocity of the virtual kelp float is a combination of the current velocity and the reduction in velocity due to drag such that, $$V = V_c + V_k $$
...
Raft-time or sometimes referred to as raft-longevity is the time a macroalgae raft remians afloat at the surface of the ocean after detachment. Raft-time was estimated from past studies by @graiff2016 and @graiff2013; which found mean raft times of approximately 30 days in moderate conditions. 
