...
p_RK4 <- tidync("OceanParcels/trajectory_data/PassiveFloat.nc") %>%
  hyper_tibble() %>%
...
p_EE <- tidync("OceanParcels/trajectory_data/PassiveFloat.nc") %>%
  hyper_tibble() %>%
  dplyr::select(lon, lat, time, distance, age) %>%
  dplyr::rename(t = time) %>%
  mutate(t = as.POSIXct(t, origin = "2002-01-01 00:00:00")) 
...
k_app1_mean <- k_app2 %>% # summarise data based on time 
   group_by(t) %>% 
   summarise_all(funs(mean = mean, sd = sd, min = min, max = max), na.rm = T)
...
total_data <- rbind(p_RK4, k_app1, k_app2)
...
library(ggpubr)
plot(total_traj)
ggarrange(heatmap_RK4,heatmap_APP1, heatmap_APP2, labels = c("A","B","C"), common.legend = TRUE, legend = c("right"))
