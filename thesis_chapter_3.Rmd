...
ggsave(filename = "figure_3.jpg", plot = last_plot(), path = "figures/chapter_3/", height = 10, width = 10, dpi = 300, scale = 2.0)
```

## Comparison of density distributions

Across all simulations for January (Austral summer), particles flowed in a north-westward direction. The density plots for simulations that considered any form of drag showed a higher density of particles along the mean trajectory path compared to the passive simulation which has an almost even density of particles across grids (Figure 4). In addition, the density plots show that the particles in simulations that considered any form of drag got entrained in a vorticity while particles in the passive simulation deflected away (Figure 4). Comparison of cross-sectional areas and different drag exposure scenarios show no differences in mean trajectory and spatial patterns in density, with only slight variations across types. 

In April and across all simulations, particles flowed in a north-westward direction (Figure 5). The density plots that considered any form of drag showed narrow, higher density along the mean trajectory compared to passive simulation. In addition, simulations that considered any form of drag got entrained within an eddy further north-westward from the end location of passive particles. Only slight variations across cross-sectional areas and drag exposure scenarios are evident. Simulations for July showed that the flow was also north-westward, but all kelp particles across cross-sectional areas and drag scenarios show high density of particles entrained in an eddy located westward of the release site compared to passive particles (Figure 6). Variation in density along the mean trajectory and eddy for all kelp particles is evident. Kelp particles with only hydrodynamic drag across cross-sectional areas showed lower density of particles along the mean trajectory compared to kelp particles with both wind and hydrodynamic drag. Simulations run for October show no difference in density of particles along the mean trajectory when comparing passive and kelp particles. Only slight variations in density and trajectory can be seen with no entrainment (Figure 7). Kelp particles with any cross-sectional area and drag exposure scenario flowed further offshore when compared to passive particles. 
...
ggsave(filename = "figure_4.jpg", plot = last_plot(), path = "figures/chapter_3/", height = 10, width = 10, scale = 1, dpi = 300)
```

```{r echo=FALSE, message=FALSE, warning=FALSE, fig.cap='Figure 5: Comparison of density plots at the end of the run for scenarios 14 - 26, for April 2018 (autumn). See Figure 4 for further details.', fig.height=8, fig.width=8}
...
ggsave(filename = "figure_5.jpg", plot = last_plot(), path = "figures/chapter_3/", height = 10, width = 10, scale = 1, dpi = 300)
```

```{r echo=FALSE, message=FALSE, warning=FALSE, fig.cap='Figure 6: Comparison of density plots at the end of the run for scenarios 27 - 39, for July 2018 (winter). See Figure 4 for further details.', fig.height=8, fig.width=8}
...
ggsave(filename = "figure_6.jpg", plot = last_plot(), path = "figures/chapter_3/", height = 10, width = 10, scale = 1, dpi = 300)
```

```{r echo=FALSE, message=FALSE, warning=FALSE, fig.cap='Figure 7: Comparison of density plots at the end of the run for scenarios 40 - 52, for October 2018 (spring). See Figure 4 for further details.', fig.height=8, fig.width=8}
...
ggsave(filename = "figure_7.jpg", plot = last_plot(), path = "figures/chapter_3/", height = 10, width = 10, scale = 1, dpi = 300)
```

## Comparison of distances 

There were significant differences among the median distances traveled for all particles in each simulation (Figure 8; *p* < 0.05). The simulations that considered for 100% hydrodynamic drag traveled a shorter distance than particles for simulations which considered a combination of hydrodynamic and wind drag scenarios (Figure 8; *p* < 0.05). Comparison between simulations that considered hydrodynamic and wind drag show no significant differences with only slight variations among simulations. When comparing distances between passive and kelp particles for April, significant differences are evident. (Figure 8, simulations 14-26; *p* < 0.05). Kelp particles with only 90% hydrodynamic and 10% wind drag across cross-sectional areas traveled shorter distance across compared to other kelp particles. The shortest distance traveled was the scenario with the maximum cross-sectional area and drag exposure scenarios. No significant differences are evident for the July simulations, with only slight variation across simulations (Figure 8, simulations 27-39; *p* > 0.05). In October, the plot shows significant differences when comparing both passive and kelp particles (Figure 8, simulations 40-52; *p* < 0.05). Kelp particles with only 90% hydrodynamic drag across cross-sectional areas traveled shorter distance compared to all other particles. As seen in the April simulations, the kelp particles with the maximum cross-sectional area and drag exposure traveled the shortest distance compared to all other particles.
...
```{r accumulation data outputs, warning=FALSE, echo=FALSE, message=FALSE, fig.cap="Figure 9: Comparison of density between passive and kelp accumulation for scenarios 53-60. For each season, passive particles are plotted on the left column and kelp particles in the right.", fig.height=8, fig.width=8}

ggarrange(heatmap_RK4_acc, heatmap_kelp_acc,heatmap_RK4_acc_April, heatmap_kelp_acc_April, heatmap_RK4_acc_July, heatmap_kelp_acc_July, heatmap_RK4_acc_Oct, heatmap_kelp_acc_Oct, common.legend = TRUE, legend = c("bottom"), ncol = 2, nrow = 4, labels = c("January","", "April","","July","","October"), font.label = list(size = 12, face = "bold"), align = "hv", label.x = 0.2)

ggsave(filename = "figure_9.jpg", plot = last_plot(), path = "figures/chapter_3/", height = 10, width = 10, scale = 2, dpi = 300)
```
...
ggarrange(point_RK4_acc, point_kelp_acc, point_RK4_acc_April, point_kelp_acc_April, point_RK4_acc_July, point_kelp_acc_July, point_RK4_acc_Oct, point_kelp_acc_Oct, common.legend = TRUE, legend = c("bottom"), ncol = 2, nrow = 4, labels = c("January", "", "April", "","July", "", "October"), font.label = list(size = 12, face = "bold"), align = "hv", label.x = 0.2)
ggsave(filename = "figure_10.jpg", plot = last_plot(), path = "figures/chapter_3/", height = 10, width = 10, scale = 2, dpi = 300)
```

The role of eddys that exist within the study domain over the time period play a clear role in the accumulation and dispersal of kelp particles. Passive particles tend to remain on the outer edge of the eddy and then are expelled on the other side and continue to follow the ocean current (Figure 9). The opposite is true for kelp particles which have the highest exposure level of hydrodynamic and wind drag. The kelp particles are entrained in the eddy (Figure 9). This is also reflected when comparing the age of particles at the end of the simulation. In Figure 10, younger particles are near the release site and older particles further away from shore, while older kelp particles are retained in the eddy. 
...
```{r shape data outputs, warning=FALSE, echo=FALSE, message=FALSE, fig.cap="Figure 11: Comparison of trajectory data between sphere and cylinder shape coefficients for scenarios 61-64. Row A represents the mean trajectory comparison, row B-C are density comparison plots, and row D is comparison of total distance travelled from the release site for each of the respective months (seasons) simulated.  The reader is referred to tables 1 and 2 for details pertaining to each scenario.", fig.height=8, fig.width=6}

shape_plot_A <- ggarrange(shape_plot, shape_plot_April, shape_plot_July, shape_plot_Oct, common.legend = TRUE, legend = c("bottom"), ncol = 4, nrow = 1, labels = c("A", "", "", ""), font.label = list(size = 12, face = "bold"), align = "hv") 
...
ggarrange(shape_plot_A, shape_plot_B, shape_dis_plot, ncol = 1, nrow = 3, legend = c("right"), labels = c("", "D") , font.label = list(size = 11, face = "bold"), heights = c(2, 2, 1))

#ggsave(filename = "figure_11.jpg", plot = last_plot(), path = "figures/chapter_3/", height = 10, width = 10, scale = 1, dpi = 300)

```

```{r vector maps, warning=FALSE, echo=FALSE, message=FALSE, fig.cap="Figure 11: The average ocean (left column) and wind vector maps (right column) for the respective seasons are presented. The velocity vectors were averaged over the 30 day simulation times for respective seasons (summer, autumn, winter, spring). A represents summer, B autumn, C winter and D spring", fig.height=8, fig.width=8}
