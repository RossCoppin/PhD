...
# load and data wrangle for eddy simulation 
...
Min_KFloat_Eddy <- tidync("OceanParcels/trajectory_data/Min_KFloat_Eddy.nc") %>%
  hyper_tibble() %>%
  dplyr::select(lon, lat, time) %>%
  dplyr::rename(t = time) %>%
  mutate(t = as.POSIXct(t, origin = "2018-01-01 00:00:00")) 
Min_K_Eddy_mean <- Min_KFloat_Eddy %>% # calculate mean trajectory
  group_by(t) %>% 
  summarise(across(lon:lat, mean, na.rm= TRUE))

Min_KFloat_Eddy$type <- "Min_KFloat_Eddy" # add extra column 'type' to dataframe 

Mean_KFloat_Eddy <- tidync("OceanParcels/trajectory_data/Mean_KFloat_Eddy.nc") %>%
  hyper_tibble() %>%
  dplyr::select(lon, lat, time) %>%
  dplyr::rename(t = time) %>%
  mutate(t = as.POSIXct(t, origin = "2018-01-01 00:00:00")) 
Mean_KFloat_Eddy_mean <- Mean_KFloat_Eddy %>% # calculate mean trajectory
  group_by(t) %>% 
  summarise(across(lon:lat, mean, na.rm= TRUE))

Mean_KFloat_Eddy$type <- "Mean_KFloat_Eddy" # add extra column 'type' to dataframe

Max_KFloat_Eddy <- tidync("OceanParcels/trajectory_data/Max_KFloat_Eddy.nc") %>%
  hyper_tibble() %>%
  dplyr::select(lon, lat, time) %>%
  dplyr::rename(t = time) %>%
  mutate(t = as.POSIXct(t, origin = "2018-01-01 00:00:00")) 
Max_KFloat_Eddy_mean <- Max_KFloat_Eddy %>% # calculate mean trajectory
  group_by(t) %>% 
  summarise(across(lon:lat, mean, na.rm= TRUE))

Max_KFloat_Eddy$type <- "Max_KFloat_Eddy" # add extra column 'type' to dataframe
...
```{r eddy plots, message=FALSE, warning=FALSE, include=TRUE, echo=FALSE}

total_eddy <- rbind(p_eddy, Min_KFloat_Eddy, Mean_KFloat_Eddy, Max_KFloat_Eddy)

ggplot() + 
  geom_point(data = total_eddy, aes(x = lon, y = lat, color = type)) +
  xlab("Longtitude") +
  ylab("Latitude") +
...
p_single <- tidync("OceanParcels/trajectory_data/RK4_Single.nc") %>%
  hyper_tibble() %>%
  dplyr::select(lon, lat, time, distance) %>%
  dplyr::rename(t = time) %>%
  mutate(t = as.POSIXct(t, origin = "2018-01-01 00:00:00"))

Min_Single_H100<- tidync("OceanParcels/trajectory_data/Min_Single_H100_W00.nc") %>%
  hyper_tibble() %>%
  dplyr::select(lon, lat, time, distance) %>%
  dplyr::rename(t = time) %>%
  mutate(t = as.POSIXct(t, origin = "2018-01-01 00:00:00")) 

Min_Single_H095_H05 <- tidync("OceanParcels/trajectory_data/Max_KFloat_H095_W05.nc") %>%
  hyper_tibble() %>%
  dplyr::select(lon, lat, time, distance) %>%
  dplyr::rename(t = time) %>%
  mutate(t = as.POSIXct(t, origin = "2018-01-01 00:00:00")) 

Min_Single_H090_H10 <- tidync("OceanParcels/trajectory_data/Min_Single_H090_W10.nc") %>%
  hyper_tibble() %>%
  dplyr::select(lon, lat, time, distance) %>%
  dplyr::rename(t = time) %>%
  mutate(t = as.POSIXct(t, origin = "2018-01-01 00:00:00")) 

Min_Single_H085_H15 <- tidync("OceanParcels/trajectory_data/Min_Single_H085_W15.nc") %>%
  hyper_tibble() %>%
  dplyr::select(lon, lat, time, distance) %>%
  dplyr::rename(t = time) %>%
  mutate(t = as.POSIXct(t, origin = "2018-01-01 00:00:00")) 
